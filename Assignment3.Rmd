...
```{r}
No_initial_report <- filter(Demographics, i_f_code != "I")
...
```{r}
#We'll be looking at the frequency of drugnames to see the top 10 which are most frequently mentioned. First need to lower to lowercase to find as many common terms and trim the whitespace
Drug$drugname <- tolower(Drug$drugname)
Drug$drugname <- trimws(Drug$drugname)
#Then we list the top 10 recurring drug names. 
count(Drug, drugname, sort = TRUE)
#Top 10 is humira, letairis, enbrel, tecifidera, dianeal low calcium, methotrexate, avonex, prednisone, lipitor, and tysabir.
test <- filter(Drug, drugname == "humira" | drugname == "letairis" | drugname == "enbrel" | drugname == "tecifidera" | drugname == "dianeal low calcium" | drugname == "methotrexate" | drugname == "avonex" | drugname == "prednisone" | drugname == "lipitor" | drugname == "tysabir")
